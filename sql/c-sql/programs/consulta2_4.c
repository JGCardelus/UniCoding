...

{
	...
	{
		//SELECT * FROM TEMPLE WHERE NOMEM LIKE '%PEREZ%'	
		strcpy(nomem,"PEREZ");//Suponemos es el nombre que pido buscar

		strcpy(query,"SELECT * FROM TEMPLE WHERE NOMEM LIKE '%");
		strcat(query,nomem);
		strcat(query,"%'");
		//printf("\n %s \n", query); // muestro la cadena  query como prueba 
